...
declare global {
  ...
  var __matchday_security_cleanup_interval_set: boolean | undefined;
}
...
if (!globalThis.__matchday_security_cleanup_interval_set) {
  setInterval(() => {
    const now = Date.now();
  ...
  globalThis.__matchday_security_cleanup_interval_set = true;
}
