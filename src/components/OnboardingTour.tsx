...
export default function OnboardingTour({ onComplete, isLoggedIn = false }: OnboardingTourProps) {
  ...
  useEffect(() => {
    ...
    const done = localStorage.getItem('matchday_tour_completed');
    if (!done) {
  ...
  const finish = () => {
    localStorage.setItem('matchday_tour_completed', 'true');
    setVisible(false);
    onComplete?.();
  };
...
export function useResetTour() {
  return () => {
    localStorage.removeItem('matchday_tour_completed');
