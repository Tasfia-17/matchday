...
export default function VirtualPet() {
    ...
    useEffect(() => {
        ...
        const savedPet = localStorage.getItem('matchday_virtual_pet');
        if (savedPet && POKEMON_DATA[savedPet]) {
            setPetFamily(savedPet);
        }

        const savedName = localStorage.getItem('matchday_virtual_pet_name');
        if (savedName) {
    ...
    const changePet = (newPet: string) => {
        setPetFamily(newPet);
        localStorage.setItem('matchday_virtual_pet', newPet);
        // Reset custom name when changing species
        setCustomName('');
        localStorage.removeItem('matchday_virtual_pet_name');
    };
    ...
    const saveName = () => {
        ...
        if (newName) {
            ...
            localStorage.setItem('matchday_virtual_pet_name', newName);
        } else {
            setCustomName('');
            localStorage.removeItem('matchday_virtual_pet_name');
        }
