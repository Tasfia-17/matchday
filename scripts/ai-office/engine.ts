// ═══════════════════════════════════════════════════════════════
//  MatchDay AI Office — Conversation Engine
...
const TOOL_DECLARATIONS: FunctionDeclaration[] = [
  ...
  },
  {
    ...
    description:
      'Read a file from the MatchDay project codebase. Use this to verify your understanding of the actual code before suggesting changes. You MUST use this tool before making code suggestions.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        filepath: {
...
export async function runOffice(
  ...
  log(`${C.yellow}${C.bold}  ║           🏢 MatchDay AI Office — Session           ║${C.reset}`);
  log(`${C.yellow}${C.bold}  ╚══════════════════════════════════════════════════════╝${C.reset}`);
  log('');
  ...
  log(`${C.dim}📂 Reading MatchDay codebase for context...${C.reset}`);
...
`;
  ...
  const reportContent = `# 🏢 MatchDay Office Report
## Task: ${task}
...
`;
  ...
  log('');
  divider('═', 60);
  log(`${C.yellow}${C.bold}  🏢 MatchDay AI Office — Session Complete${C.reset}`);
  divider('═', 60);
  log('');
